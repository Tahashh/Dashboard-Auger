...
import { fetchMacchina5000, addMacchina5000, updateMacchina5000, deleteMacchina5000, addMovementLog, addMovimentoCGialla } from '../api';
import { toast } from 'react-hot-toast';
import { Plus, Save, Trash2, CheckCircle, Clock, Play, Square, Loader2, RefreshCw, Edit2, X } from 'lucide-react';
import clsx from 'clsx';

interface Macchina5000Props {
  articles: Article[];
  username: string;
  role: string;
  onUpdate: () => void;
...
export default function Macchina5000({ articles, username, role, onUpdate }: Macchina5000Props) {
  const [items, setItems] = useState<Macchina5000Type[]>([]);
  const [loading, setLoading] = useState(true);
  
  // Confirmation state
  const [confirmingItem, setConfirmingItem] = useState<Macchina5000Type | null>(null);
  ...
  const isAndrea = username === 'Andrea' || username === 'TahaDev';
  ...
  const loadData = async (silent = false) => {
    try {
      ...
      const data = await fetchMacchina5000();
      setItems(data);
    } catch (error: any) {
      toast.error('Errore nel caricamento dati Macchina 5000: ' + error.message);
    } finally {
  ...
  const handleStart = async (item: Macchina5000Type, prep: number) => {
    try {
      const timeStr = new Date().toISOString();
      await updateMacchina5000(item.id, {
        inizio: timeStr,
  ...
  const handlePause = async (item: Macchina5000Type) => {
    try {
      const timeStr = new Date().toISOString();
      console.log('Sending pause update:', { id: item.id, pausa: timeStr });
      await updateMacchina5000(item.id, {
        pausa: timeStr,
        stato: 'in pausa',
        operatore: username
  ...
  const handleResume = async (item: Macchina5000Type) => {
    try {
      const timeStr = new Date().toISOString();
      await updateMacchina5000(item.id, {
        inizio2: timeStr,
  ...
  const handleEnd = async (item: Macchina5000Type, prep: number) => {
    try {
      const timeStr = new Date().toISOString();
      await updateMacchina5000(item.id, {
        fine: timeStr,
  ...
  const handleComplete = async (item: Macchina5000Type, prep: number, fine: string, confirmedQty: number) => {
    try {
      ...
      if (articleObj) {
        ...
        await addMovementLog({
          ...
          fase: 'Tagliato - Carico da Macchina 5000',
          tipo: 'carico',
          quantita: confirmedQty,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-2">
        <h1 className="text-2xl font-bold text-slate-800">Macchina 5000</h1>
        <button
          onClick={() => loadData()}
          ...
          <form onSubmit={handleAddProgram} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                required
              />
              <datalist id="articles-list-m5000">
                {articles.map(a => (
              ...
              {items.map((item) => (
                <Macchina5000Row 
                  ...
                  onUpdateRow={async (id, data) => {
                    try {
                      await updateMacchina5000(id, data);
...
function Macchina5000Row({ 
  ...
  onDelete: () => void;
  isDeveloper: boolean;
  onUpdateRow: (id: string, data: Partial<Macchina5000Type>) => Promise<void>;
}) {
  const [prep, setPrep] = useState<number | ''>(item.preparazione || '');
  const [isEditing, setIsEditing] = useState(false);
  const [editData, setEditData] = useState<Partial<Macchina5000Type>>({});
