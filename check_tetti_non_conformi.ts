...
try {
  ...
  const nonMatchingTetti = tetti.filter(t => !t.nome.toUpperCase().startsWith('PANN. TETTO AGR'));
  
  console.log('Tetti non conformi:', nonMatchingTetti.length);
  nonMatchingTetti.forEach(t => console.log(`${t.nome} - ${t.codice}`));
} catch (e) {
  console.error(e);
}
